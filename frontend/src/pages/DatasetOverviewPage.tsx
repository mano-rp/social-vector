  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-lg bg-white dark:bg-[#0f141c] border border-slate-200 dark:border-slate-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 rounded-lg bg-white dark:bg-[#0f141c] border border-slate-200 dark:border-slate-800 space-y-2.5">
              ...
              {topTags.map(([tag, count]) => {
                ...
                return (
                  <div
                    key={tag}
                    onClick={() => navigate(`/datasets/${datasetId}/feed`)}
                    className="p-2 rounded bg-slate-50 dark:bg-slate-900/40 hover:bg-slate-100 dark:hover:bg-slate-800/60 cursor-pointer transition-colors space-y-1"
                  >
                    <div className="flex items-center justify-between text-xs font-mono">
                      <span className="text-blue-600 dark:text-cyan-400 font-medium">#{tag}</span>
                      <span className="text-slate-500">{count} posts</span>
                    </div>
                    <div className="w-full h-1 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-600 dark:bg-cyan-400 rounded-full" style={{ width: `${widthPct}%` }} />
                    </div>
                  </div>
                );
              })}
            ...
            <div className="space-y-1.5">
              {topDomains.map(([domain, count]) => {
                ...
                return (
                  <div
                    key={domain}
                    className="p-2 rounded bg-slate-50 dark:bg-slate-900/40 space-y-1"
                  >
                    <div className="flex items-center justify-between text-xs font-mono">
                    ...
                    <div className="w-full h-1 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-slate-700 dark:bg-slate-400 rounded-full" style={{ width: `${widthPct}%` }} />
                    </div>
                  </div>
                );
              })}
