...
class OrganicActivityScenario(BaseScenario):
    ...
    def run(self) -> Tuple[List[UserRecord], List[PostRecord], Optional[GroundTruth]]:
        ...
        # Distribute post counts per user using Pareto-distributed activity levels
        for u in users:
            # Individual activity level (some users post 1-2 times, some post more)
